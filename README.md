...
**The floor comes off the same bracket as everything else.** The walk-away figure sits a fixed margin below the estimate, so it describes the property being sold rather than an average-sized one — a market-wide basis would carry the same understatement the bracket exists to correct. That has a consequence worth knowing: the price-to-compete strategy can land *below* the floor, because it drops to the search-band edge beneath the estimate and the estimate can sit just above one. When it does, it stops being the recommendation. Advertising a price under the floor asks the seller to refuse the offer they invited, so between a tactic and a limit the limit wins — pricing at market is recommended instead, the compete price stays on the page relabelled, and the run names the only lever that would change the answer: lowering the floor, if less would really be accepted.
...
**The agent analysis is a shortlist to interview, not a ranking.** Samples are one to three sales per agent, so the ratio against ask is a median rather than a mean — one bidding war moves an average of three sales a long way. A high ratio can reflect a deliberately low list price as much as skill. The default order is the interview order: the flagged shortlist first, then closings, then that ratio, with volume ahead of ratio because a single sale bid up 22% says less than three sales at par. The recommended interview test is in the workbook: ask each candidate for a list price *and* an expected close, before showing them your numbers.
...
the command line does. 392 tests, all offline; a live run needs no test to
pass.
